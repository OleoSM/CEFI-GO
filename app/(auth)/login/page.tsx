...
export default function LoginPage() {
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    e.preventDefault();
    setLoading(true);
    // Dev bypass — Supabase no disponible localmente
    ...
    <div className="min-h-screen flex items-center justify-center px-4 py-16 relative">
      ...
      >
        ...
        <div className="rounded-2xl bg-white/[0.04] border border-white/10 backdrop-blur-xl p-8 shadow-2xl shadow-black/40">
          ...
          <p className="text-white/40 text-sm text-center mb-5">Ingresa a tu cuenta para continuar</p>
          ...
          <div className="flex gap-2 mb-6">
            {QUICK_USERS.map((u) => (
              <button
                ...
                className="flex-1 py-1.5 rounded-lg text-xs font-semibold bg-white/5 border border-white/10 text-white/50 hover:bg-violet-500/15 hover:border-violet-500/30 hover:text-violet-300 transition-all"
              >
                {u.label}
              </button>
            ))}
          ...
          <form noValidate onSubmit={handleSubmit} className="space-y-4">
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full py-3 rounded-xl text-sm font-bold bg-gradient-to-r from-violet-600 to-pink-600 text-white hover:opacity-90 active:opacity-80 transition-opacity shadow-lg shadow-violet-900/30 disabled:opacity-60 mt-2"
            >
